<?php
    ...
    */
    ...
    if(isset($_POST['limitpatient'])) {
        $limit = mysqli_real_escape_string($db->conn, $_POST['limitpatient']);
        $output = '';
        $post_data = $db->execute_limitData('patient', $limit, 'field', 0);
        foreach ($post_data as $post) {
            $output .= '<tr>
                 <td>'.$post["id"].'</td>
                 <td>'.$post["name"].'</td>
    ...
    */
    ...
    if(isset($_POST['nrselect'])) {
        $idnr = mysqli_real_escape_string($db->conn, $_POST['id']);
        $output = array();
     ...
     if(isset($_POST['limitnurses'])) {
        $limit = mysqli_real_escape_string($db->conn, $_POST['limitnurses']);
        $output = '';
...
//		$post_data = $db->execute_query('doctors', null);
    	foreach ($post_data as $post) {
    		$output .= '<tr>
    			<td>'.$post["id"].'</td>
                <td>'.$post["name"].'</td>
    ...
    if(isset($_POST['searchdoctor'])) {
        $search = mysqli_real_escape_string($db->conn, $_POST['search']);
        $output = '';
        $query = "SELECT dr.id, dr.name, dep.name AS depname, dr.phone, dr.gender, dr.status, dr.shift, dr.salary FROM doctors AS dr, department AS dep WHERE (dr.name LIKE '%$search%' or
            dr.phone LIKE '%$search%' or dr.id LIKE '%$search%') AND dr.dep_id = dep.id ORDER BY dr.id DESC";
        $post_data = $db->execute_sql($query);
        foreach ($post_data as $post) {
            $output .= '<tr>
            <td>'.$post["id"].'</td>
    ...
    if(isset($_POST['limitdoctor'])) {
        $limit = mysqli_real_escape_string($db->conn, $_POST['limitdoctor']);
        $output = '';
        $query = "SELECT dr.id, dr.name, department.name AS depname, dr.phone, dr.gender, dr.status, dr.shift,
         dr.salary FROM doctors AS dr, department WHERE dr.dep_id = department.id";
    ...
    // lists the doctors to assign a patient
    ...
    */
    ...
    if(isset($_POST['asslimit'])) {
        $limit = mysqli_real_escape_string($db->conn, $_POST['asslimit']);
        $output = '';
        $query = "SELECT assign.id, patient.name AS patient, doctors.name AS doctor, examine, symptoms 
        FROM assign, patient, doctors WHERE assign.patient_id = patient.id AND assign.doctor_id = doctors.id
        AND patient.field = 1";
